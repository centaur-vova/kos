...
## Почему Behat + Gherkin

Для проверки сценариев использован **Behat с Gherkin-синтаксисом** — тесты читаются как документация:

- **Человекочитаемые сценарии** — понятное текстовое описание всех шагов бизнес-логики
- **Покрытие требований** — каждый пункт ТЗ отражен в отдельном сценарии
- **Воспроизводимость** — `make test` прогоняет все 21 сценарий за секунды
- **Изоляция** — каждый сценарий сбрасывает БД и конфигурацию провайдеров
